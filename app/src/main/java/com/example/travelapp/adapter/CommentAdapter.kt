...
class CommentAdapter:BaseExpandableListAdapter() {
    private var commentList: MutableList<CommentVo> = ArrayList()
    private var context: Context? = null
    var onItemClick:((CommentVo)->Unit)?=null
    ...
    override fun getChildView(p0: Int, p1: Int, p2: Boolean, p3: View?, p4: ViewGroup?): View {
        ...
        if(p1==0&&commentList[p0].child.size>1||commentList[p0].child[p1].child!=null){
            childHolder.seeMore.visibility = View.VISIBLE
            childHolder.seeMore.setOnClickListener {
                onItemClick?.invoke(commentList[p0])
            }
        }else{
            childHolder.seeMore.visibility = View.GONE
        }
        return convertView!!
    }
    ...
    inner class ChildHolder(view: View){
        ...
        var seeMore:TextView = view.findViewById(R.id.see_more)
    }

    fun addTheCommentData(commentVo: CommentVo) {
        if(commentList!=null){
    ...
    fun addTheReplyData(commentVo: CommentVo, groupPosition: Int) {
        if (commentList[groupPosition].child != null) {
            commentList[groupPosition].child.add(commentVo)
        } else {
            val replyList: MutableList<CommentVo> = ArrayList()
            replyList.add(commentVo)
            commentList[groupPosition].child = replyList
        }
        notifyDataSetChanged()
    }
}